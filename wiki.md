...
Then, they have the following data fields accessible:
...
- `void Image::render(SDL_Renderer *renderer, int x_pos, int y_pos)`: renders the image at a given position `(x_pos,y_pos)`. By default, they're set to the origin point, else to the set position.
