...
Image::Image(SDL_Renderer *renderer, const char* image_path, const Vector2D& pos ,int width, int height, SDL_Color i_color) noexcept : i_renderer(renderer), path(image_path), w(width), h(height), color(i_color), position(pos) {
